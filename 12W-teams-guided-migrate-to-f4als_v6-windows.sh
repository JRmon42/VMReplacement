...
if [ "$DRYRUN" != 1 ]; then
  ...
  # ignores existing unallocated space and insists on shrinking C: itself. This is documented
  # first-party behaviour, not a quirk: https://learn.microsoft.com/en-us/windows/deployment/mbr-to-gpt
  ...
  if grep -q "CONVERT_FAIL" <<<"$PREP" && grep -q "SHRINK=OK" <<<"$PREP"; then
    die "C: was shrunk successfully, but mbr2gpt still cannot create the EFI partition." "This is the known dead end of running mbr2gpt inside a live Windows (/allowFullOS): it logs 'System partition cannot be removed in full OS mode, leaving it untouched', so it refuses to reuse the existing 500MB 'System Reserved' partition, and it will not use free space it did not create itself. The conversion has to be done with the disk OFFLINE, from WinRE on this VM (reagentc /boottore + restart, then 'mbr2gpt /convert /disk:0'), where that restriction does not apply and the System Reserved partition itself becomes the EFI partition. CHECK WinRE IS ACTUALLY AVAILABLE FIRST - run 'reagentc /info' (must report Enabled) and confirm C:\Windows\System32\Recovery\Winre.wim exists. Azure Windows marketplace images ship WITHOUT that recovery image, and you cannot borrow it from another Azure VM for the same reason; it has to be extracted from Windows Server installation media before this route can even start. Note also that an Azure VM cannot boot from an ISO, so WinRE is the ONLY way to reach a WinPE environment, and it is driven over Serial Console. Note that attaching the OS disk to a helper VM does NOT work: mbr2gpt only accepts the system disk. Contact us and we will drive the WinRE procedure with you. $ALT_GEN1_HINT NOTHING destructive has run - the VM still boots and snapshot '$SNAPSHOT' remains your rollback point."
  fi
  case "$PREP" in
    ...
    *CONVERT_FAIL*)   die "mbr2gpt /convert failed (diagnostics captured above)." "mbr2gpt validated the disk but could not create the ~100MB EFI partition. In full-OS mode it ALWAYS carves the ESP by shrinking C: - it will not reuse the existing 500MB 'System Reserved' partition ('System partition cannot be removed in full OS mode'), and it ignores unallocated space that already exists on the disk, so growing the disk does not help either - Microsoft documents this at https://learn.microsoft.com/en-us/windows/deployment/mbr-to-gpt ('Creating an EFI system partition': the MBR system partition is reused only when the conversion is NOT performed from the full OS, otherwise the ESP is created by shrinking the OS partition). 'Partition final size is <n> (initial size was <n>)' means the shrink freed nothing because an immovable file sits at the end of C:. This is the dead end of the in-OS route: the conversion has to be done with the disk OFFLINE, from WinRE - but first confirm WinRE exists here ('reagentc /info' must say Enabled and C:\Windows\System32\Recovery\Winre.wim must be present), because Azure marketplace Windows images ship without it and it then has to come from installation media. $ALT_GEN1_HINT Send us the whole output above - or, more simply, the in-guest log: az vm run-command invoke -g $RG -n $VM --command-id RunPowerShellScript --scripts \"Get-Content C:\\Windows\\Temp\\mig-prep-last.log -Tail 80\" --query \"value[0].message\" -o tsv    NOTHING destructive has run - snapshot '$SNAPSHOT' remains your rollback point.";;
