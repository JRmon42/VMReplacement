#!/usr/bin/env bash
# GROUP B - OPTION B2: Blue/green from snapshot. Lowest SERVICE interruption + instant rollback.
# Builds a NEW Gen2/NVMe v6 VM from the OS-disk snapshot; original keeps serving until cutover.
#
# NOTE (NVMe readiness): the v6 sizes require the NVMe disk controller. Two things must be true
# or the staging VM will fail to create/boot:
...
SRC_VM="vm-ds11v2"
TARGET="Standard_E2s_v6"     # or Standard_D2s_v6 for the DS1_v2 box
OS_SNAPSHOT="vm-ds11v2-os-YYYYMMDD-HHMMSS"   # from 00-snapshot-all.sh
...
echo "==> Staging VM up. VALIDATE the app on '$STAGE_VM' before cutover."

if [[ "$PRESERVE_NAME" != "true" ]]; then
  cat <<NOTE
  Cutover (keep NEW name '$STAGE_VM'):
    1. Stop the app on the OLD VM '$SRC_VM'.
    2. Move the private IP (reassign NIC ipconfig) OR repoint DNS / LB backend pool to '$STAGE_VM'.
    3. Reattach any data disks (detach from old, attach to new) if not snapshot-copied.
...
cat <<'NOTE'
  Post-cutover:
    1. Reattach any data disks to the recreated VM (detach from old disk set / attach by ID).
    2. Re-apply VM extensions, boot diagnostics, tags, and identities not carried by the disk/NIC.
    3. Verify the app, then clean up the OLD OS disk and snapshots after a safe rollback window.
